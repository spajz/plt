...
class AdjacencyApi
{
    ...
    public function getTree($id = null, $root = false)
    {
        ...
        $items = $model::filter()->orderBy('sort', 'asc')->get();
    ...
    public function itemsToTree($items, $parentId = 0)
    {
        ...
        foreach ($items as $item) {
            if ($item['parent_id'] == $parentId) {
                $item['count'] = null;
                if (Session::get('options.category.count_products')) {
                    $counter = $this->getRelatedItemCounter();
                    if ($counter) {
                        $item['count'] = $counter($item);
                    }
                }
                // Todo Group ID 0
    ...
    public function setRelatedItemCounter($closure)
    {
        $this->relatedItemCounter = $closure;
    }

    public function getRelatedItemCounter()
    {
        return $this->relatedItemCounter;
    }

    public function processItemElements()
    {
